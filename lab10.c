...
char history_save(const history *, const char *);
...
hist_node * _history_node_create(const char *, char * eK);
void _history_node_free(hist_node **);
int _free_history_nodes_rec(hist_node ** h);
char update_encryption_keys(history * h, const char * fname);
// history tests
void run_tests();
...
void usage()
{
    ...
    printf("    u : update encryption key and save\n");
    ...
    while(1){
        ...
        switch(command){
            ...
            case 'P':
                ...
                break;
            case 's':
                //printf("using the key %s", key);
                if(history_save(h, fname)){
            ...
            case 'C':
                ...
                break;
            case 'u':
                update_encryption_keys(h, fname);
                break;
            case 'r':
                history_free(&h);
 ...
 */
hist_node * _history_node_create(const char * url, char * eK){
...
char update_encryption_keys(history * h, const char * fname){
    if(h->head){
        char *key = (char *) malloc(sizeof(char) * (2049));
        hist_node *n = (hist_node *) malloc(sizeof(hist_node));
        printf("Please input new key: ");
        scanf("%s", key);
        n=h->head;
        n->encryption_key = key;
        h->encryption_key = key;
        while(n->next){
            n=n->next;
            n->encryption_key = key;
        }
        printf("Encryption key successfully changed to %s, saving now.\n", n->encryption_key);
        n->hasUpdatedKey=1;
        history_save(h, fname);
        return 0;
    } else
        printf("Please visit a url before updating key\n");
    return 1;
...
char history_add(history * h, const char * url){
    ...
    // create new node
    hist_node * n = _history_node_create(url, "stuff");
    if(!n){
        return 0;
    }
    if(h->current){
...
void history_print(const history * h){//////////////////////////////////////////////////////////
    ...
    if(h->head->isEnc){
        char key[25];
        printf("Please input decryption key\n");
        scanf("%s", key);
        //printf("Madeit1");
        hist_node *n = h->head;
        //printf("Key is %s", key);
        ...
        if(n){
            do{
                ...
                //u= (char*)malloc(sizeof(char)*(strlen(n->url)+1));
                for(int i=0; i<12; ++i){
                    n->url[i]^=(~key[i % keyLength]);
                }
                n->isEnc=0;
                n=n->next;
            }while(n);
 ...
 */
char _history_write_node(hist_node * n, int nidx, FILE * fout)
{
    if(!n || !fout){
        return 0;
    }
    ...
    for(int i=0; i<strlen(n->url); ++i){
        n->url[i]^=(~key[i % keyLength]);
    }
    n->isEnc=1;
    if(fwrite(n->url, sizeof(char), l, fout) != l){
        return 0;
    }
    // done
    return 1;
 ...
 */
char history_save(const history * h, const char * fname){
    
    char *key = (char *) malloc(sizeof(char) * (2049));
    hist_node *n = (hist_node *) malloc(sizeof(hist_node));
    n=h->head;
    if(n->hasUpdatedKey!=1){
        printf("Please input encryption key: ");
        scanf("%s", key);
        n->encryption_key = key;
        while(n->next){
            n=n->next;
...
//    int *ie = (int*) malloc(sizeof(int));
//    ie = &(h->head->isEnc);
//    fwrite(&ie, sizeof(int), 1, fout);
//
...
hist_node * _history_read_node(FILE * fin, char *key){
    if(!fin){
        return NULL;
    }
...
history * history_read(const char * fname)
{
    ...
    scanf("%s", key);
    // add nodes
    hist_node * n = _history_read_node(fin, key);
    h->head = h->tail = n;
    if(cidx == 0){
        h->current = n;
    }
    for(int i=1; i < size; ++i){
        n = _history_read_node(fin, key);
        if(!n){
            history_free(&h);
            return NULL;
        }
